...
impl IpcConnection {
    ...
    ///
    /// Polls the iceoryx2 subscriber and falls back to [`POLL_INTERVAL`]
    /// sleeps until a message arrives. A caller may explicitly opt into a
    /// bounded active-yield window with [`ACTIVE_POLL_BUDGET_MS_ENV`].
    ///
    /// # Errors
    ///
    /// - [`TransportError::ShuttingDown`] if the connection is closed.
