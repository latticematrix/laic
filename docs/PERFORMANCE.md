...
Each evidence table below is owned by the release section that contains it. These measurements are not a production SLA, not a maximum-capacity claim, and not a promise for every deployment topology.

## 0.2.0 Release Note

The published `0.2.0` line does not overwrite the `0.1.0 MVP` performance table. Its theme is performance and usability validation closeout:

- keep the reviewed `0.1.0` performance evidence version-marked;
- record the Windows-local IPC optimization as bounded `0.2.0` current evidence, not a new WAN/LAN authority;
- add public CI and release smoke as release evidence for documented onboarding and fail-closed paths.
...
## 0.2.0 Windows Local Evidence

This table belongs to `0.2.0`. It records the post-optimization Windows-local validation path only. It does not replace the reviewed `0.1.0 MVP` evidence, and it does not promote a new LAN, WAN, hosted, or production-SLA claim.
...
- Local loopback validation established the baseline local transport path for the MVP line.
...
| Windows local validation | moved to `0.2.0 Windows Local Evidence` for the post-optimization table |
